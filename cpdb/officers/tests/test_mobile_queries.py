...
from officers.queries import OfficerTimelineMobileQuery
...
class OfficerTimelineMobileQueryTestCase(TestCase):
    ...
    @patch(
        'officers.queries.OfficerTimelineMobileQuery.lawsuit_new_timeline_serializer',
        return_value=Mock(data=[{'id': 1}, {'id': 2}])
    )
    def test_lawsuit_timeline(self, lawsuit_new_timeline_serializer_mock):
        ...
        expect(OfficerTimelineMobileQuery(officer)._lawsuit_timeline).to.eq([{'id': 1}, {'id': 2}])
    ...
    @patch(
        'officers.queries.OfficerTimelineMobileQuery._lawsuit_timeline',
        new_callable=PropertyMock,
        return_value=[
