...
class OfficersViewSet(viewsets.ViewSet):
    ...
    def _query_new_timeline_items(self, request, pk):
        if request.GET.get('sort') == 'asc':
    ...
    def new_timeline_items(self, request, pk):
        if Officer.objects.filter(pk=pk).exists():
            query = self._query_new_timeline_items(request, pk)
            return Response(NewTimelineSerializer(query.execute(), many=True).data)
        return Response(status=status.HTTP_404_NOT_FOUND)
