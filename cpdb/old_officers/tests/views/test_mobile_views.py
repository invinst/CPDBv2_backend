...
class OfficersMobileViewSetTestCase(OfficerSummaryTestCaseMixin, APITestCase):
    ...
    def test_retrieve_data_range_too_small_cause_no_percentiles(self):
        ...
        response = self.client.get(reverse('api-v2:officers-mobile-old-detail', kwargs={'pk': 123}))
        expected_response = {
            'officer_id': 123,
            'unit': {
                'unit_id': 1,
    ...
    def test_retrieve_no_match(self):
        self.refresh_index()
        response = self.client.get(reverse('api-v2:officers-mobile-old-detail', kwargs={'pk': 456}))
        expect(response.status_code).to.eq(status.HTTP_404_NOT_FOUND)

    def test_new_timeline_items_no_match(self):
        response = self.client.get(reverse('api-v2:officers-mobile-old-new-timeline-items', kwargs={'pk': 456}))
        expect(response.status_code).to.eq(status.HTTP_404_NOT_FOUND)
    ...
    def test_new_timeline_item(self):
        ...
        self.refresh_index()
        response = self.client.get(reverse('api-v2:officers-mobile-old-new-timeline-items', kwargs={'pk': 123}))
