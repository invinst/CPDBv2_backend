...
class UpdateOfficerAllegationManager(UpdateManagerBase):
    ...
    def query_data(self):
        return f"""
                select
                    ...
                    coalesce(substring(nullif(trim(recc_finding), ''), 1, 2), substring(nullif(final_finding, ''), 1, 2), 'ZZ') as recc_finding,
                    ...
                    concat_ws('/', nullif(trim(t.category_tier_2), ''), nullif(trim(t.category_tier_3), ''), nullif(trim(t.category_tier_4), '')) as allegation_name,
                ...
                left join (
                    ...
                    on (case when nullif(t.category_tier_1, '') is null then trim(t.complaint_code) else 'no match' end) = c.category_code
                order by
                    crid, officer_id
    ...
    def process_batch(self, batch):
        ...
        grouped_allegations = {}
        officer_allegation_keys = ['allegation_id', 'officer_id', 'final_outcome', 'recc_outcome', 'disciplined', 'start_date', 'end_date']
        ...
        officer_allegations = [{k: v for k, v in allegation_group[0].items() 
                                if k in officer_allegation_keys}
                                for allegation_group in grouped_allegations.values()]
        ...
        try:
            findings = [{"officer_allegation_id": officer_allegation_ids[(key[0], key[1])],
                        "final_finding": finding['final_finding'],
                        "recc_finding": finding['recc_finding'],
                        "allegation_category_id": new_categories[(finding['category_code'], finding['category'], finding['allegation_name'])]
                            if finding['category'] 
    ...
    def add_or_get_categories(self, grouped_allegations):
        ...
        if new_format_rows:
            for category_code, category, allegation_name in set((row['category_code'], row['category'], row['allegation_name']) for row in new_format_rows):
    ...
    def update_holding_table(self):
        ...
        cursor = connection.cursor()
        cursor.execute("create index if not exists idx_csv_final_profiles_uid_int on csv_final_profiles((uid::float::int));")
        cursor.execute("create index if not exists idx_csv_complaints_accused_uid_int on csv_complaints_accused((uid::float::int));")
