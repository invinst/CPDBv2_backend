...
def import_data(apps, schema_editor):
    AllegationCategory = apps.get_model('data', 'AllegationCategory')
    blank_or_null = {
    ...
    with csv_from_azure('20180530_allegationcategory.csv') as reader:
        for row in reader:
            ...
            pk = int(row.pop('pk'))
            pks.append(pk)
            obj, created = AllegationCategory.objects.update_or_create(
                id=pk,
                defaults=row
            )

    AllegationCategory.objects.exclude(pk__in=pks).delete()
