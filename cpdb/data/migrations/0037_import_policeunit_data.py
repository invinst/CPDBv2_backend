...
def import_data(apps, schema_editor):
    ...
    blank_or_null = {
        ...
        for field in PoliceUnit._meta.get_fields()
    }
    boolean_fields = [
        field.name for field in PoliceUnit._meta.get_fields()
    ...
    with csv_from_azure('20180522_policeunit.csv') as reader:
        for row in reader:
            ...
            pk = int(row.pop('pk'))
            pks.append(pk)
            obj, created = PoliceUnit.objects.update_or_create(
                id=pk,
                defaults=row
            )

    PoliceUnit.objects.exclude(pk__in=pks).delete()


class Migration(migrations.Migration):

    dependencies = [
        ('data', '0036_policeunit_active'),
    ]
