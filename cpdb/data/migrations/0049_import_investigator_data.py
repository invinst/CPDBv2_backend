# -*- coding: utf-8 -*-
# Generated by Django 1.11.4 on 2018-05-30 08:44
...
def import_data(apps, schema_editor):
    Investigator = apps.get_model('data', 'Investigator')
    blank_or_null = {
        '%s_id' % field.name if field.is_relation else field.name: None if field.null else ''
        for field in Investigator._meta.get_fields()
    }
    pks = []

    with csv_from_azure('20180530_investigator.csv') as reader:
        for row in reader:
            ...
            pk = int(row.pop('pk'))
            pks.append(pk)
            obj, created = Investigator.objects.update_or_create(
                id=pk,
                defaults=row
            )

    Investigator.objects.exclude(pk__in=pks).delete()


class Migration(migrations.Migration):

    dependencies = [
        ('data', '0048_investigator'),
    ]
