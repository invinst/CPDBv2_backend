...
from data.models import OfficerBadgeNumber
...
def bulk_create_update(klass, rows):
    ...
    new_objects = [klass(**row) for row in rows if row['id'] not in object_dict]
    old_objects = [
...
def import_data(apps, schema_editor):
    with csv_from_azure('20181001_officerbadgenumber.csv') as reader:
        blank_or_null = {
        ...
        def clean(row):
            ...
            for key in row.keys():
                if key not in ['id', 'officer_id', 'star', 'current']:
                    row.pop(key)
        ...
        bulk_create_update(OfficerBadgeNumber, rows)


class Migration(migrations.Migration):

    dependencies = [
        ('data', '0070_import_officerhistory_data_20181001'),
    ]
