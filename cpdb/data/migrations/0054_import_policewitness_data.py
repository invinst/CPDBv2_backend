...
def import_data(apps, schema_editor):
    PoliceWitness = apps.get_model('data', 'PoliceWitness')
    blank_or_null = {
    ...
    with csv_from_azure('20180531_policewitness.csv') as reader:
        for row in reader:
            ...
            pk = int(row.pop('pk'))
            pks.append(pk)
            obj, created = PoliceWitness.objects.update_or_create(
                id=pk,
                defaults=row
            )

    PoliceWitness.objects.exclude(pk__in=pks).delete()


class Migration(migrations.Migration):

    dependencies = [
        ('data', '0053_import_complainant_data'),
    ]
