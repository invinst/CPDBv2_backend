...
class AllegationWithNewDocumentsSerializer(NoNullSerializer):
    ...
    category = serializers.CharField(source='allegation_obj.most_common_category.category', allow_null=True)
    incident_date = serializers.DateTimeField(
        source='allegation_obj.incident_date', format='%Y-%m-%d', default_timezone=pytz.utc
    )
