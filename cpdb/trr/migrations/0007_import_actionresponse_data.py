...
def import_data(apps, schema_editor):
    ActionResponse = apps.get_model('trr', 'ActionResponse')
    blank_or_null = {
        field.get_attname(): None if field.null else ''
        for field in ActionResponse._meta.get_fields()
        if hasattr(field, 'get_attname')
    }
    pks = []

    with csv_from_azure('20180518_actionresponse.csv') as reader:
        for row in reader:
            ...
            pk = int(row.pop('pk'))
            pks.append(pk)
            obj, created = ActionResponse.objects.update_or_create(
                id=pk,
                defaults=row
            )

    ActionResponse.objects.exclude(pk__in=pks).delete()


class Migration(migrations.Migration):

    dependencies = [
        ('trr', '0006_import_trr_data'),
    ]
