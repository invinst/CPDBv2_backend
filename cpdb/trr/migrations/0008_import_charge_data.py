...
def import_data(apps, schema_editor):
    Charge = apps.get_model('trr', 'Charge')
    blank_or_null = {
    ...
    with csv_from_azure('20180518_charge.csv') as reader:
        for row in reader:
            ...
            pk = int(row.pop('pk'))
            pks.append(pk)
            obj, created = Charge.objects.update_or_create(
                id=pk,
                defaults=row
            )

    Charge.objects.exclude(pk__in=pks).delete()


class Migration(migrations.Migration):

    dependencies = [
        ('trr', '0007_import_actionresponse_data'),
    ]
