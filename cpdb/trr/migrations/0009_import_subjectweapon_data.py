...
def import_data(apps, schema_editor):
    SubjectWeapon = apps.get_model('trr', 'SubjectWeapon')
    blank_or_null = {
        field.get_attname(): None if field.null else ''
        for field in SubjectWeapon._meta.get_fields()
        if hasattr(field, 'get_attname')
    }
    pks = []

    with csv_from_azure('20180518_subjectweapon.csv') as reader:
        for row in reader:
            ...
            pk = int(row.pop('pk'))
            pks.append(pk)
            obj, created = SubjectWeapon.objects.update_or_create(
                id=pk,
                defaults=row
            )

    SubjectWeapon.objects.exclude(pk__in=pks).delete()


class Migration(migrations.Migration):

    dependencies = [
        ('trr', '0008_import_charge_data'),
    ]
