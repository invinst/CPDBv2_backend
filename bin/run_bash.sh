...
export JOB_NAME
export COMMAND=$JOB_COMMAND
export ARGS="$(echo $REST | sed 's/ /\", \"/g')"

cat kubernetes/bash.yml | envsubst | kubectl delete -f - -n $NAMESPACE || true
...
JOB_STATUS="$(cat kubernetes/bash.yml | envsubst | kubectl apply -f - --namespace $NAMESPACE)"
echo $JOB_STATUS
