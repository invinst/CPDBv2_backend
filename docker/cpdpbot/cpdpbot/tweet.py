...
class TweetSender(object):
    ...
    def __init__(self):
        ...
        self.fail_queue_name = f'{self.queue_name}fail'
        ...
        self.print_stdout(f'Logged in as @{twitter_user.screen_name}')
        ...
        self.print_stdout(f'Created queue {self.queue_name}')
        ...
        for message in self.queue_service.get_messages(self.queue_name, num_messages=1):
            try:
                ...
                if filename is not None:
                    ...
                    self.print_stdout(f"Sent status with media: {data['tweet']['status']} - {self.status_url(status)}")
                else:
                    status = self.twitter_api.update_status(**data['tweet'])
                    self.print_stdout(f"Sent status: {data['tweet']['status']} - {self.status_url(status)}")
            except Exception:
                self.print_exception(f'Cannot send status: {content}')
